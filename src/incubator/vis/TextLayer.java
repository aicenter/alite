...
public class TextLayer extends CommonLayer {
    ...
    public void paint(Graphics2D canvas) {
        ...
        //compute text height
        int textHeight = (1 + texts.getTexts().size()) * fontMetrics.getHeight();


        canvas.setColor(new Color(0, 0, 0, 200));
        canvas.fillRect(Vis.transX(texts.getPosition().x), Vis.transY(texts.getPosition().y), textWidth, textHeight);
        ...
        int x = Vis.transX(texts.getPosition().x) + fontMetrics.charWidth(SPACING_CHARACTER);
