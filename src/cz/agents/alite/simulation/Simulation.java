...
/**
 * The simulation is a wrapper of the {@link EventProcessor} with an additional functionality
 * covering adjustable simulation speed.
 * 
 * An agent should never get a reference to the simulation. The simulation should be directly used
 * only by other services of the universe (environment, communication, etc). The agent is controlled
 * by the simulation only indirectly: by {@link Sensor} callbacks, message receiving, and so on.
 * 
 * 
...
public class Simulation extends EventProcessor {
    ...
    /**
     * request drawing frame, if not running, returns false and not draws it ! if running, draws
     * after some time
     */
    ...
    /**
     * sets min interval between drawings, time for simulation to reload for new drawing
     */
    ...
    protected void breforeRunningTest(Event event) {
        ...
        if (eventCount % printouts == 0) {
            System.out.format(">>> TIME: %ds / RUNTIME: %.2fs / EVENTS: %d / QUEUE: %d \n",
                    getCurrentTime() / 1000, (System.currentTimeMillis() - runTime) / 1000.0,
    ...
    private long drawFrame(long timeToSleep) {
        ...
        final long drawDeadline = System.currentTimeMillis() + Math.max(timeToSleep, drawTimeout);
        ...
        try {
            ...
            Logger.getLogger(EventProcessor.class.getName()).log(Level.INFO, null, e);
        }
