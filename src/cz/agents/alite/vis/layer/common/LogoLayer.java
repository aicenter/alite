...
public class LogoLayer extends ImageLayer {
    ...
    public void paint(Graphics2D canvas) {
        ...
        for (Image image : getImageElements().getImages()) {
            canvas.drawImage(image.getImage(), Vis.getDrawingDimension().width - image.getImage().getWidth() - 25, -10, null);
            imageHeight = image.getImage().getHeight();
        }

        canvas.setColor(Color.BLUE);
        canvas.drawString("Press F1 for help", Vis.getDrawingDimension().width - 115, imageHeight);
    }
