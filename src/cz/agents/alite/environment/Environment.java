...
public abstract class Environment {
     ...
     * generator, which should be used for any randomization in the logic of the
     * entities/agents (if it it so, it is possible to create deterministically
     * reproducible runs of the system - the random seed can be set at one
     * place).
     * 
     * 
     * @author Antonin Komenda
	...
	/**
	 * The method creates a new instance of the <code>clazz</code> in the
	 * environment.
	 * 
	 * The instance is created using a constructor, which is compatible with
	 * the parameter types in the <code>baseParameterTypesRequired</code>
	 * array. By the compatibility, it is meant the argument must be of the
	 * type or of a subtype of the type.
	 * 
	 * @param <C>
	 ...
	 *            an array of the types according which the constructor
	 *            should be used
	 * @return the newly instantiated environment class (sensor/action
	 *         typically)
	 */
	@SuppressWarnings("unchecked")
	protected <C> C instantiateEnvironmentClass(Class<C> clazz,
		Entity entity, Class<?>[] baseParameterTypesRequired) {
	    C instance = null;
		...
		for (Constructor<?> constructor : clazz.getConstructors()) {
			...
			if (!baseParameterTypesRequired[i]
		...
		if (selectedConstructor != null) {
		    instance = selectedConstructor.newInstance(
			    Environment.this, entity);
		} else {
		    throw new RuntimeException(
			    "Cannot find an usable constructor in class "
				    + clazz.getCanonicalName()
				    + " for base parameter types: "
